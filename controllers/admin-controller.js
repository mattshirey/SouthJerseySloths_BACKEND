...
const createGames = async (req, res, next) => {
	...
	try {
		...
		const error = new HttpError(
			'Could not find league to obtain leagueId. createGame 1',
			500
		)
		return next(error)
	}
	...
	if (leagueName2) {
		...
		let leagueId2, divisionName
		let foundLeague2, foundLeague2WithDivisions
		try {
			foundLeague2 = await League.findOne({
				leagueName: leagueName2,
				session: session2,
				year: year2,
			}).orFail()
		} catch (err) {
		...
		leagueId2 = foundLeague2.id
		divisionName = foundLeague2.divisionName
		// We have our leagueId, so let's use that to find team names...
		//find home team Id
		if (divisionName) {
			console.log('we have divisions here!')
			try {
				foundLeague2WithDivisions = await League.find({
					leagueName: leagueName2,
					session: session2,
					year: year2,
				}).orFail()
				...
				const error = new HttpError(
					'Could not find league to obtain leagueId. WITH divisions. createGame 2',
					500
				)
				return next(error)
			}
			//console.log('foundLeague2WithDivisions: ' + foundLeague2WithDivisions)
			//So we have multiple leagues here.  Let's make an array of all teams
			let allTeamsInThisLeague, teams
			//MATT START
			allTeamsInThisLeague = []
			for (let i = 0; i < foundLeague2WithDivisions.length; i++) {
				try {
					teams = await Team.find({
						leagueName: foundLeague2WithDivisions[i].leagueName,
						divisionName: foundLeague2WithDivisions[i].divisionName,
					}).orFail()
				} catch {}
				teams.forEach((team) => {
					allTeamsInThisLeague.push(team)
			...
			//console.log('allTeamsInThisLeague: ' + allTeamsInThisLeague)
			//
			//
			// BEGIN COPY HERE
			//
			...
			if (homeTeam2 !== 'TBD') {
				allTeamsInThisLeague.forEach(async (team) => {
					if (team.teamName === homeTeam2) {
						foundHomeTeam2 = team
					}
				})
				//console.log('foundHomeTeam2: ' + foundHomeTeam2)
				homeTeamId2 = foundHomeTeam2.id
			}
			//
			//
			//find visitor team id
			let visitorTeamId2
			let foundVisitorTeam2
			if (visitorTeam2 !== 'TBD') {
				allTeamsInThisLeague.forEach(async (team) => {
					//console.log(team.leagueName)
					console.log(team.teamName + '  ' + visitorTeam2)
					console.log(team.divisionName + ' ' + divisionName)

					if (team.teamName === visitorTeam2) {
						foundVisitorTeam2 = team
					}
				})
				//console.log('foundVisitorTeam!: ' + foundVisitorTeam2)
				visitorTeamId2 = foundVisitorTeam2.id
			}
			//
			//
			//
			//Now let's find the roster id's
			let homeTeamRosterId2
			let foundHomeTeamRosterId2
			if (homeTeam2 !== 'TBD') {
				try {
					foundHomeTeamRosterId2 = await Roster.findOne({
						leagueId: foundHomeTeam2.leagueId,
						teamId: homeTeamId2,
						divisionName: foundHomeTeam2.divisionName,
					}).orFail()
				} catch (err) {
					const error = new HttpError(
						'Could not find roster to obtain home rosterId. WITH divisions.  createGame 2',
						500
					)
					return next(error)
				}
				homeTeamRosterId2 = foundHomeTeamRosterId2.id
			}

			//// find visitor team roster id
			let visitorTeamRosterId2
			let foundVisitorTeamRosterId2
			if (visitorTeam2 !== 'TBD') {
				try {
					foundVisitorTeamRosterId2 = await Roster.findOne({
						leagueId: foundVisitorTeam2.leagueId,
						teamId: visitorTeamId2,
						divisionName: foundVisitorTeam2.divisionName,
					}).orFail()
				} catch (err) {
					const error = new HttpError(
						'Could not find roster to obtain home rosterId. WITH divisions.  createGame 2',
				...
				visitorTeamRosterId2 = foundVisitorTeamRosterId2.id
			}
			//
			//
			//
			...
			//Now we should have all the variables we need to create a new game
			if (homeTeam2 !== 'TBD' && visitorTeam2 !== 'TBD') {
				createdGame2 = new Game({
					...
					homeRosterId: homeTeamRosterId2,
					homeTeamSeed: foundHomeTeam2.seed,
					visitorTeamName: visitorTeam2,
					visitorTeamId: visitorTeamId2,
					visitorRosterId: visitorTeamRosterId2,
					visitorTeamSeed: foundVisitorTeam2.seed,
					venueName: venue2,
					isCurrent: true,
				})
				try {
				...
				//If only the home team is TBD:
			} else if (homeTeam2 === 'TBD' && visitorTeam2 !== 'TBD') {
				console.log('22d')
				createdGame2 = new Game({
					leagueName: leagueName2,
					session: session2,
			...
			console.log('Game 2 has been added')
		} else if (!divisionName) {
			//
			//
			//
			...
			//
			//
			console.log('no divisions for this game 2')

			let homeTeamId2
			let foundHomeTeam2
			if (homeTeam2 !== 'TBD') {
				try {
					foundHomeTeam2 = await Team.findOne({
						leagueId: leagueId2,
						teamName: homeTeam2,
					}).orFail()
				} catch (err) {
					const error = new HttpError(
						'Could not find team to obtain home teamId. createGame 2',
						500
					)
					return next(error)
				}
			...
			if (visitorTeam2 !== 'TBD') {
				try {
					foundVisitorTeam2 = await Team.findOne({
						leagueId: leagueId2,
						teamName: visitorTeam2,
					}).orFail()
				} catch (err) {
					const error = new HttpError(
						'Could not find team to obtain visitor teamId. createGame 2',
						500
					)
					return next(error)
				}
			...
			if (homeTeam2 !== 'TBD') {
				try {
					...
					const error = new HttpError(
						'Could not find roster to obtain home rosterId. createGame 2',
						500
					)
					return next(error)
				}
			...
			if (visitorTeam2 !== 'TBD') {
				try {
					...
					const error = new HttpError(
						'Could not find roster to obtain visitor rosterId. createGame 2',
						500
					)
					return next(error)
				}
	...
	if (leagueName3) {
		...
		let leagueId3, divisionName
		let foundLeague3, foundLeague3WithDivisions
		try {
			foundLeague3 = await League.findOne({
				leagueName: leagueName3,
				session: session3,
				year: year3,
			}).orFail()
		} catch (err) {
		...
		leagueId3 = foundLeague3.id
		divisionName = foundLeague3.divisionName
		// We have our leagueId, so let's use that to find team names...
		//find home team Id
		if (divisionName) {
			console.log('we have divisions here!')
			try {
				foundLeague3WithDivisions = await League.find({
					leagueName: leagueName3,
					session: session3,
					year: year3,
				}).orFail()
				...
				const error = new HttpError(
					'Could not find league to obtain leagueId. WITH divisions. createGame 3',
					500
				)
				return next(error)
			}
			...
			//MATT START
			allTeamsInThisLeague = []
			for (let i = 0; i < foundLeague3WithDivisions.length; i++) {
				try {
					teams = await Team.find({
						leagueName: foundLeague3WithDivisions[i].leagueName,
						divisionName: foundLeague3WithDivisions[i].divisionName,
					}).orFail()
				} catch {}
				teams.forEach((team) => {
					allTeamsInThisLeague.push(team)
			...
			if (homeTeam3 !== 'TBD') {
				allTeamsInThisLeague.forEach(async (team) => {
					if (team.teamName === homeTeam3) {
						foundHomeTeam3 = team
					}
				})
				homeTeamId3 = foundHomeTeam3.id
			}
			//
			//
			//find visitor team id
			let visitorTeamId3
			let foundVisitorTeam3
			if (visitorTeam3 !== 'TBD') {
				allTeamsInThisLeague.forEach(async (team) => {
					//console.log(team.leagueName)
					console.log(team.teamName + '  ' + visitorTeam3)
					console.log(team.divisionName + ' ' + divisionName)

					if (team.teamName === visitorTeam3) {
						foundVisitorTeam3 = team
					}
				})
				visitorTeamId3 = foundVisitorTeam3.id
			}
			//
			//
			//
			//Now let's find the roster id's
			let homeTeamRosterId3
			let foundHomeTeamRosterId3
			if (homeTeam3 !== 'TBD') {
				try {
					foundHomeTeamRosterId3 = await Roster.findOne({
						leagueId: foundHomeTeam3.leagueId,
						teamId: homeTeamId3,
						divisionName: foundHomeTeam3.divisionName,
					}).orFail()
				} catch (err) {
					const error = new HttpError(
						'Could not find roster to obtain home rosterId. WITH divisions.  createGame 3',
						500
					)
					return next(error)
				}
				homeTeamRosterId3 = foundHomeTeamRosterId3.id
			}

			//// find visitor team roster id
			let visitorTeamRosterId3
			let foundVisitorTeamRosterId3
			if (visitorTeam3 !== 'TBD') {
				try {
					foundVisitorTeamRosterId3 = await Roster.findOne({
						leagueId: foundVisitorTeam3.leagueId,
						teamId: visitorTeamId3,
						divisionName: foundVisitorTeam3.divisionName,
					}).orFail()
				} catch (err) {
					const error = new HttpError(
						'Could not find roster to obtain home rosterId. WITH divisions.  createGame 3',
				...
				visitorTeamRosterId3 = foundVisitorTeamRosterId3.id
			}
			//
			//
			//
			...
			//Now we should have all the variables we need to create a new game
			if (homeTeam3 !== 'TBD' && visitorTeam3 !== 'TBD') {
				createdGame3 = new Game({
					...
					homeTeamName: homeTeam3,
					homeTeamId: homeTeamId3,
					homeRosterId: homeTeamRosterId3,
					homeTeamSeed: foundHomeTeam3.seed,
					visitorTeamName: visitorTeam3,
					visitorTeamId: visitorTeamId3,
					visitorRosterId: visitorTeamRosterId3,
					visitorTeamSeed: foundVisitorTeam3.seed,
					venueName: venue3,
					isCurrent: true,
				})
				try {
				...
				//If only the home team is TBD:
			} else if (homeTeam3 === 'TBD' && visitorTeam3 !== 'TBD') {
				console.log('23d')
				createdGame3 = new Game({
					leagueName: leagueName3,
					session: session3,
			...
			console.log('Game 3 has been added')
		} else if (!divisionName) {
			//
			//
			//
			...
			//
			//
			console.log('no divisions for this game 3')

			let homeTeamId3
			let foundHomeTeam3
			if (homeTeam3 !== 'TBD') {
				try {
					foundHomeTeam3 = await Team.findOne({
						leagueId: leagueId3,
						teamName: homeTeam3,
					}).orFail()
				} catch (err) {
					const error = new HttpError(
						'Could not find team to obtain home teamId. createGame 3',
						500
					)
					return next(error)
				}
			...
			if (visitorTeam3 !== 'TBD') {
				try {
					foundVisitorTeam3 = await Team.findOne({
						leagueId: leagueId3,
						teamName: visitorTeam3,
					}).orFail()
				} catch (err) {
					const error = new HttpError(
						'Could not find team to obtain visitor teamId. createGame 3',
						500
					)
					return next(error)
				}
			...
			if (homeTeam3 !== 'TBD') {
				try {
					...
					const error = new HttpError(
						'Could not find roster to obtain home rosterId. createGame 3',
						500
					)
					return next(error)
				}
			...
			if (visitorTeam3 !== 'TBD') {
				try {
					...
					const error = new HttpError(
						'Could not find roster to obtain visitor rosterId. createGame 3',
						500
					)
					return next(error)
				}
	...
	if (leagueName4) {
		...
		let leagueId4, divisionName
		let foundLeague4, foundLeague4WithDivisions
		try {
			foundLeague4 = await League.findOne({
				leagueName: leagueName4,
				session: session4,
				year: year4,
			}).orFail()
		} catch (err) {
		...
		leagueId4 = foundLeague4.id
		divisionName = foundLeague4.divisionName
		// We have our leagueId, so let's use that to find team names...
		//find home team Id
		if (divisionName) {
			console.log('we have divisions here!')
			try {
				foundLeague4WithDivisions = await League.find({
					leagueName: leagueName4,
					session: session4,
					year: year4,
				}).orFail()
				...
				const error = new HttpError(
					'Could not find league to obtain leagueId. WITH divisions. createGame 4',
					500
				)
				return next(error)
			}
			...
			//MATT START
			allTeamsInThisLeague = []
			for (let i = 0; i < foundLeague4WithDivisions.length; i++) {
				try {
					teams = await Team.find({
						leagueName: foundLeague4WithDivisions[i].leagueName,
						divisionName: foundLeague4WithDivisions[i].divisionName,
					}).orFail()
				} catch {}
				teams.forEach((team) => {
					allTeamsInThisLeague.push(team)
			...
			if (homeTeam4 !== 'TBD') {
				allTeamsInThisLeague.forEach(async (team) => {
					if (team.teamName === homeTeam4) {
						foundHomeTeam4 = team
					}
				})
				homeTeamId4 = foundHomeTeam4.id
			}
			//
			//
			//find visitor team id
			let visitorTeamId4
			let foundVisitorTeam4
			if (visitorTeam4 !== 'TBD') {
				allTeamsInThisLeague.forEach(async (team) => {
					//console.log(team.leagueName)
					console.log(team.teamName + '  ' + visitorTeam4)
					console.log(team.divisionName + ' ' + divisionName)

					if (team.teamName === visitorTeam4) {
						foundVisitorTeam4 = team
					}
				})
				visitorTeamId4 = foundVisitorTeam4.id
			}
			//
			//
			//
			//Now let's find the roster id's
			let homeTeamRosterId4
			let foundHomeTeamRosterId4
			if (homeTeam4 !== 'TBD') {
				try {
					foundHomeTeamRosterId4 = await Roster.findOne({
						leagueId: foundHomeTeam4.leagueId,
						teamId: homeTeamId4,
						divisionName: foundHomeTeam4.divisionName,
					}).orFail()
				} catch (err) {
					const error = new HttpError(
						'Could not find roster to obtain home rosterId. WITH divisions.  createGame 4',
						500
					)
					return next(error)
				}
				homeTeamRosterId4 = foundHomeTeamRosterId4.id
			}

			//// find visitor team roster id
			let visitorTeamRosterId4
			let foundVisitorTeamRosterId4
			if (visitorTeam4 !== 'TBD') {
				try {
					foundVisitorTeamRosterId4 = await Roster.findOne({
						leagueId: foundVisitorTeam4.leagueId,
						teamId: visitorTeamId4,
						divisionName: foundVisitorTeam4.divisionName,
					}).orFail()
				} catch (err) {
					const error = new HttpError(
						'Could not find roster to obtain home rosterId. WITH divisions.  createGame 4',
				...
				visitorTeamRosterId4 = foundVisitorTeamRosterId4.id
			}
			//
			//
			//
			...
			//Now we should have all the variables we need to create a new game
			if (homeTeam4 !== 'TBD' && visitorTeam4 !== 'TBD') {
				createdGame4 = new Game({
					...
					homeTeamName: homeTeam4,
					homeTeamId: homeTeamId4,
					homeRosterId: homeTeamRosterId4,
					homeTeamSeed: foundHomeTeam4.seed,
					visitorTeamName: visitorTeam4,
					visitorTeamId: visitorTeamId4,
					visitorRosterId: visitorTeamRosterId4,
					visitorTeamSeed: foundVisitorTeam4.seed,
					venueName: venue4,
					isCurrent: true,
				})
				try {
				...
				//If only the home team is TBD:
			} else if (homeTeam4 === 'TBD' && visitorTeam4 !== 'TBD') {
				console.log('24d')
				createdGame4 = new Game({
					leagueName: leagueName4,
					session: session4,
			...
			console.log('Game 4 has been added')
		} else if (!divisionName) {
			//
			//
			//
			...
			//
			//
			console.log('no divisions for this game 4')

			let homeTeamId4
			let foundHomeTeam4
			if (homeTeam4 !== 'TBD') {
				try {
					foundHomeTeam4 = await Team.findOne({
						leagueId: leagueId4,
						teamName: homeTeam4,
					}).orFail()
				} catch (err) {
					const error = new HttpError(
						'Could not find team to obtain home teamId. createGame 4',
						500
					)
					return next(error)
				}
			...
			if (visitorTeam4 !== 'TBD') {
				try {
					foundVisitorTeam4 = await Team.findOne({
						leagueId: leagueId4,
						teamName: visitorTeam4,
					}).orFail()
				} catch (err) {
					const error = new HttpError(
						'Could not find team to obtain visitor teamId. createGame 4',
						500
					)
					return next(error)
				}
			...
			if (homeTeam4 !== 'TBD') {
				try {
					...
					const error = new HttpError(
						'Could not find roster to obtain home rosterId. createGame 4',
						500
					)
					return next(error)
				}
			...
			if (visitorTeam4 !== 'TBD') {
				try {
					...
					const error = new HttpError(
						'Could not find roster to obtain visitor rosterId. createGame 4',
						500
					)
					return next(error)
				}
	...
	if (leagueName5) {
		...
		let leagueId5, divisionName
		let foundLeague5, foundLeague5WithDivisions
		try {
			foundLeague5 = await League.findOne({
				leagueName: leagueName5,
				session: session5,
				year: year5,
			}).orFail()
		} catch (err) {
		...
		leagueId5 = foundLeague5.id
		divisionName = foundLeague5.divisionName
		// We have our leagueId, so let's use that to find team names...
		//find home team Id
		if (divisionName) {
			console.log('we have divisions here!')
			try {
				foundLeague5WithDivisions = await League.find({
					leagueName: leagueName5,
					session: session5,
					year: year5,
				}).orFail()
				...
				const error = new HttpError(
					'Could not find league to obtain leagueId. WITH divisions. createGame 5',
					500
				)
				return next(error)
			}
			...
			//MATT START
			allTeamsInThisLeague = []
			for (let i = 0; i < foundLeague5WithDivisions.length; i++) {
				try {
					teams = await Team.find({
						leagueName: foundLeague5WithDivisions[i].leagueName,
						divisionName: foundLeague5WithDivisions[i].divisionName,
					}).orFail()
				} catch {}
				teams.forEach((team) => {
					allTeamsInThisLeague.push(team)
			...
			if (homeTeam5 !== 'TBD') {
				allTeamsInThisLeague.forEach(async (team) => {
					if (team.teamName === homeTeam5) {
						foundHomeTeam5 = team
					}
				})
				homeTeamId5 = foundHomeTeam5.id
			}
			//
			//
			//find visitor team id
			let visitorTeamId5
			let foundVisitorTeam5
			if (visitorTeam5 !== 'TBD') {
				allTeamsInThisLeague.forEach(async (team) => {
					//console.log(team.leagueName)
					console.log(team.teamName + '  ' + visitorTeam5)
					console.log(team.divisionName + ' ' + divisionName)

					if (team.teamName === visitorTeam5) {
						foundVisitorTeam5 = team
					}
				})
				visitorTeamId5 = foundVisitorTeam5.id
			}
			//
			//
			//
			//Now let's find the roster id's
			let homeTeamRosterId5
			let foundHomeTeamRosterId5
			if (homeTeam5 !== 'TBD') {
				try {
					foundHomeTeamRosterId5 = await Roster.findOne({
						leagueId: foundHomeTeam5.leagueId,
						teamId: homeTeamId5,
						divisionName: foundHomeTeam5.divisionName,
					}).orFail()
				} catch (err) {
					const error = new HttpError(
						'Could not find roster to obtain home rosterId. WITH divisions.  createGame 5',
						500
					)
					return next(error)
				}
				homeTeamRosterId5 = foundHomeTeamRosterId5.id
			}

			//// find visitor team roster id
			let visitorTeamRosterId5
			let foundVisitorTeamRosterId5
			if (visitorTeam5 !== 'TBD') {
				try {
					foundVisitorTeamRosterId5 = await Roster.findOne({
						leagueId: foundVisitorTeam5.leagueId,
						teamId: visitorTeamId5,
						divisionName: foundVisitorTeam5.divisionName,
					}).orFail()
				} catch (err) {
					const error = new HttpError(
						'Could not find roster to obtain home rosterId. WITH divisions.  createGame 5',
				...
				visitorTeamRosterId5 = foundVisitorTeamRosterId5.id
			}
			//
			//
			//
			...
			//Now we should have all the variables we need to create a new game
			if (homeTeam5 !== 'TBD' && visitorTeam5 !== 'TBD') {
				createdGame5 = new Game({
					...
					homeTeamName: homeTeam5,
					homeTeamId: homeTeamId5,
					homeRosterId: homeTeamRosterId5,
					homeTeamSeed: foundHomeTeam5.seed,
					visitorTeamName: visitorTeam5,
					visitorTeamId: visitorTeamId5,
					visitorRosterId: visitorTeamRosterId5,
					visitorTeamSeed: foundVisitorTeam5.seed,
					venueName: venue5,
					isCurrent: true,
				})
				try {
				...
				//If only the home team is TBD:
			} else if (homeTeam5 === 'TBD' && visitorTeam5 !== 'TBD') {
				console.log('25d')
				createdGame5 = new Game({
					leagueName: leagueName5,
					session: session5,
			...
			console.log('Game 5 has been added')
		} else if (!divisionName) {
			//
			//
			//
			...
			//
			//
			console.log('no divisions for this game 5')

			let homeTeamId5
			let foundHomeTeam5
			if (homeTeam5 !== 'TBD') {
				try {
					foundHomeTeam5 = await Team.findOne({
						leagueId: leagueId5,
						teamName: homeTeam5,
					}).orFail()
				} catch (err) {
					const error = new HttpError(
						'Could not find team to obtain home teamId. createGame 5',
						500
					)
					return next(error)
				}
			...
			if (visitorTeam5 !== 'TBD') {
				try {
					foundVisitorTeam5 = await Team.findOne({
						leagueId: leagueId5,
						teamName: visitorTeam5,
					}).orFail()
				} catch (err) {
					const error = new HttpError(
						'Could not find team to obtain visitor teamId. createGame 5',
						500
					)
					return next(error)
				}
			...
			if (homeTeam5 !== 'TBD') {
				try {
					...
					const error = new HttpError(
						'Could not find roster to obtain home rosterId. createGame 5',
						500
					)
					return next(error)
				}
			...
			if (visitorTeam5 !== 'TBD') {
				try {
					...
					const error = new HttpError(
						'Could not find roster to obtain visitor rosterId. createGame 5',
						500
					)
					return next(error)
				}
	...
	if (leagueName6) {
		...
		let leagueId6, divisionName
		let foundLeague6, foundLeague6WithDivisions
		try {
			foundLeague6 = await League.findOne({
				leagueName: leagueName6,
				session: session6,
				year: year6,
			}).orFail()
		} catch (err) {
		...
		leagueId6 = foundLeague6.id
		divisionName = foundLeague6.divisionName
		// We have our leagueId, so let's use that to find team names...
		//find home team Id
		if (divisionName) {
			console.log('we have divisions here!')
			try {
				foundLeague6WithDivisions = await League.find({
					leagueName: leagueName6,
					session: session6,
					year: year6,
				}).orFail()
				...
				const error = new HttpError(
					'Could not find league to obtain leagueId. WITH divisions. createGame 6',
					500
				)
				return next(error)
			}
			...
			//MATT START
			allTeamsInThisLeague = []
			for (let i = 0; i < foundLeague6WithDivisions.length; i++) {
				try {
					teams = await Team.find({
						leagueName: foundLeague6WithDivisions[i].leagueName,
						divisionName: foundLeague6WithDivisions[i].divisionName,
					}).orFail()
				} catch {}
				teams.forEach((team) => {
					allTeamsInThisLeague.push(team)
			...
			if (homeTeam6 !== 'TBD') {
				allTeamsInThisLeague.forEach(async (team) => {
					if (team.teamName === homeTeam6) {
						foundHomeTeam6 = team
					}
				})
				homeTeamId6 = foundHomeTeam6.id
			}
			//
			//
			//find visitor team id
			let visitorTeamId6
			let foundVisitorTeam6
			if (visitorTeam6 !== 'TBD') {
				allTeamsInThisLeague.forEach(async (team) => {
					//console.log(team.leagueName)
					console.log(team.teamName + '  ' + visitorTeam6)
					console.log(team.divisionName + ' ' + divisionName)

					if (team.teamName === visitorTeam6) {
						foundVisitorTeam6 = team
					}
				})
				visitorTeamId6 = foundVisitorTeam6.id
			}
			//
			//
			//
			//Now let's find the roster id's
			let homeTeamRosterId6
			let foundHomeTeamRosterId6
			if (homeTeam6 !== 'TBD') {
				try {
					foundHomeTeamRosterId6 = await Roster.findOne({
						leagueId: foundHomeTeam6.leagueId,
						teamId: homeTeamId6,
						divisionName: foundHomeTeam6.divisionName,
					}).orFail()
				} catch (err) {
					const error = new HttpError(
						'Could not find roster to obtain home rosterId. WITH divisions.  createGame 6',
						500
					)
					return next(error)
				}
				homeTeamRosterId6 = foundHomeTeamRosterId6.id
			}

			//// find visitor team roster id
			let visitorTeamRosterId6
			let foundVisitorTeamRosterId6
			if (visitorTeam6 !== 'TBD') {
				try {
					foundVisitorTeamRosterId6 = await Roster.findOne({
						leagueId: foundVisitorTeam6.leagueId,
						teamId: visitorTeamId6,
						divisionName: foundVisitorTeam6.divisionName,
					}).orFail()
				} catch (err) {
					const error = new HttpError(
						'Could not find roster to obtain home rosterId. WITH divisions.  createGame 6',
				...
				visitorTeamRosterId6 = foundVisitorTeamRosterId6.id
			}
			//
			//
			//
			...
			//Now we should have all the variables we need to create a new game
			if (homeTeam6 !== 'TBD' && visitorTeam6 !== 'TBD') {
				createdGame6 = new Game({
					...
					homeTeamName: homeTeam6,
					homeTeamId: homeTeamId6,
					homeRosterId: homeTeamRosterId6,
					homeTeamSeed: foundHomeTeam6.seed,
					visitorTeamName: visitorTeam6,
					visitorTeamId: visitorTeamId6,
					visitorRosterId: visitorTeamRosterId6,
					visitorTeamSeed: foundVisitorTeam6.seed,
					venueName: venue6,
					isCurrent: true,
				})
				try {
				...
				//If only the home team is TBD:
			} else if (homeTeam6 === 'TBD' && visitorTeam6 !== 'TBD') {
				console.log('26d')
				createdGame6 = new Game({
					leagueName: leagueName6,
					session: session6,
			...
			console.log('Game 6 has been added')
		} else if (!divisionName) {
			//
			//
			//
			...
			//
			//
			console.log('no divisions for this game 6')

			let homeTeamId6
			let foundHomeTeam6
			if (homeTeam6 !== 'TBD') {
				try {
					foundHomeTeam6 = await Team.findOne({
						leagueId: leagueId6,
						teamName: homeTeam6,
					}).orFail()
				} catch (err) {
					const error = new HttpError(
						'Could not find team to obtain home teamId. createGame 6',
						500
					)
					return next(error)
				}
			...
			if (visitorTeam6 !== 'TBD') {
				try {
					foundVisitorTeam6 = await Team.findOne({
						leagueId: leagueId6,
						teamName: visitorTeam6,
					}).orFail()
				} catch (err) {
					const error = new HttpError(
						'Could not find team to obtain visitor teamId. createGame 6',
						500
					)
					return next(error)
				}
			...
			if (homeTeam6 !== 'TBD') {
				try {
					...
					const error = new HttpError(
						'Could not find roster to obtain home rosterId. createGame 6',
						500
					)
					return next(error)
				}
			...
			if (visitorTeam6 !== 'TBD') {
				try {
					...
					const error = new HttpError(
						'Could not find roster to obtain visitor rosterId. createGame 6',
						500
					)
					return next(error)
				}
	...
	if (leagueName7) {
		...
		let leagueId7, divisionName
		let foundLeague7, foundLeague7WithDivisions
		try {
			foundLeague7 = await League.findOne({
				leagueName: leagueName7,
				session: session7,
				year: year7,
			}).orFail()
		} catch (err) {
		...
		leagueId7 = foundLeague7.id
		divisionName = foundLeague7.divisionName
		// We have our leagueId, so let's use that to find team names...
		//find home team Id
		if (divisionName) {
			console.log('we have divisions here!')
			try {
				foundLeague7WithDivisions = await League.find({
					leagueName: leagueName7,
					session: session7,
					year: year7,
				}).orFail()
				...
				const error = new HttpError(
					'Could not find league to obtain leagueId. WITH divisions. createGame 7',
					500
				)
				return next(error)
			}
			...
			//MATT START
			allTeamsInThisLeague = []
			for (let i = 0; i < foundLeague7WithDivisions.length; i++) {
				try {
					teams = await Team.find({
						leagueName: foundLeague7WithDivisions[i].leagueName,
						divisionName: foundLeague7WithDivisions[i].divisionName,
					}).orFail()
				} catch {}
				teams.forEach((team) => {
					allTeamsInThisLeague.push(team)
			...
			if (homeTeam7 !== 'TBD') {
				allTeamsInThisLeague.forEach(async (team) => {
					if (team.teamName === homeTeam7) {
						foundHomeTeam7 = team
					}
				})
				homeTeamId7 = foundHomeTeam7.id
			}
			//
			//
			//find visitor team id
			let visitorTeamId7
			let foundVisitorTeam7
			if (visitorTeam7 !== 'TBD') {
				allTeamsInThisLeague.forEach(async (team) => {
					//console.log(team.leagueName)
					console.log(team.teamName + '  ' + visitorTeam7)
					console.log(team.divisionName + ' ' + divisionName)

					if (team.teamName === visitorTeam7) {
						foundVisitorTeam7 = team
					}
				})
				visitorTeamId7 = foundVisitorTeam7.id
			}
			//
			//
			//
			//Now let's find the roster id's
			let homeTeamRosterId7
			let foundHomeTeamRosterId7
			if (homeTeam7 !== 'TBD') {
				try {
					foundHomeTeamRosterId7 = await Roster.findOne({
						leagueId: foundHomeTeam7.leagueId,
						teamId: homeTeamId7,
						divisionName: foundHomeTeam7.divisionName,
					}).orFail()
				} catch (err) {
					const error = new HttpError(
						'Could not find roster to obtain home rosterId. WITH divisions.  createGame 7',
						500
					)
					return next(error)
				}
				homeTeamRosterId7 = foundHomeTeamRosterId7.id
			}

			//// find visitor team roster id
			let visitorTeamRosterId7
			let foundVisitorTeamRosterId7
			if (visitorTeam7 !== 'TBD') {
				try {
					foundVisitorTeamRosterId7 = await Roster.findOne({
						leagueId: foundVisitorTeam7.leagueId,
						teamId: visitorTeamId7,
						divisionName: foundVisitorTeam7.divisionName,
					}).orFail()
				} catch (err) {
					const error = new HttpError(
						'Could not find roster to obtain home rosterId. WITH divisions.  createGame 7',
				...
				visitorTeamRosterId7 = foundVisitorTeamRosterId7.id
			}
			//
			//
			//
			...
			//Now we should have all the variables we need to create a new game
			if (homeTeam7 !== 'TBD' && visitorTeam7 !== 'TBD') {
				createdGame7 = new Game({
					...
					homeTeamName: homeTeam7,
					homeTeamId: homeTeamId7,
					homeRosterId: homeTeamRosterId7,
					homeTeamSeed: foundHomeTeam7.seed,
					visitorTeamName: visitorTeam7,
					visitorTeamId: visitorTeamId7,
					visitorRosterId: visitorTeamRosterId7,
					visitorTeamSeed: foundVisitorTeam7.seed,
					venueName: venue7,
					isCurrent: true,
				})
				try {
				...
				//If only the home team is TBD:
			} else if (homeTeam7 === 'TBD' && visitorTeam7 !== 'TBD') {
				console.log('27d')
				createdGame7 = new Game({
					leagueName: leagueName7,
					session: session7,
			...
			console.log('Game 7 has been added')
		} else if (!divisionName) {
			//
			//
			//
			...
			//
			//
			console.log('no divisions for this game 7')

			let homeTeamId7
			let foundHomeTeam7
			if (homeTeam7 !== 'TBD') {
				try {
					foundHomeTeam7 = await Team.findOne({
						leagueId: leagueId7,
						teamName: homeTeam7,
					}).orFail()
				} catch (err) {
					const error = new HttpError(
						'Could not find team to obtain home teamId. createGame 7',
						500
					)
					return next(error)
				}
			...
			if (visitorTeam7 !== 'TBD') {
				try {
					foundVisitorTeam7 = await Team.findOne({
						leagueId: leagueId7,
						teamName: visitorTeam7,
					}).orFail()
				} catch (err) {
					const error = new HttpError(
						'Could not find team to obtain visitor teamId. createGame 7',
						500
					)
					return next(error)
				}
			...
			if (homeTeam7 !== 'TBD') {
				try {
					...
					const error = new HttpError(
						'Could not find roster to obtain home rosterId. createGame 7',
						500
					)
					return next(error)
				}
			...
			if (visitorTeam7 !== 'TBD') {
				try {
					...
					const error = new HttpError(
						'Could not find roster to obtain visitor rosterId. createGame 7',
						500
					)
					return next(error)
				}
	...
	if (leagueName8) {
		...
		let leagueId8, divisionName
		let foundLeague8, foundLeague8WithDivisions
		try {
			foundLeague8 = await League.findOne({
				leagueName: leagueName8,
				session: session8,
				year: year8,
			}).orFail()
		} catch (err) {
		...
		leagueId8 = foundLeague8.id
		divisionName = foundLeague8.divisionName
		// We have our leagueId, so let's use that to find team names...
		//find home team Id
		if (divisionName) {
			console.log('we have divisions here!')
			try {
				foundLeague8WithDivisions = await League.find({
					leagueName: leagueName8,
					session: session8,
					year: year8,
				}).orFail()
				...
				const error = new HttpError(
					'Could not find league to obtain leagueId. WITH divisions. createGame 8',
					500
				)
				return next(error)
			}
			...
			//MATT START
			allTeamsInThisLeague = []
			for (let i = 0; i < foundLeague8WithDivisions.length; i++) {
				try {
					teams = await Team.find({
						leagueName: foundLeague8WithDivisions[i].leagueName,
						divisionName: foundLeague8WithDivisions[i].divisionName,
					}).orFail()
				} catch {}
				teams.forEach((team) => {
					allTeamsInThisLeague.push(team)
			...
			if (homeTeam8 !== 'TBD') {
				allTeamsInThisLeague.forEach(async (team) => {
					if (team.teamName === homeTeam8) {
						foundHomeTeam8 = team
					}
				})
				homeTeamId8 = foundHomeTeam8.id
			}
			//
			//
			//find visitor team id
			let visitorTeamId8
			let foundVisitorTeam8
			if (visitorTeam8 !== 'TBD') {
				allTeamsInThisLeague.forEach(async (team) => {
					//console.log(team.leagueName)
					console.log(team.teamName + '  ' + visitorTeam8)
					console.log(team.divisionName + ' ' + divisionName)

					if (team.teamName === visitorTeam8) {
						foundVisitorTeam8 = team
					}
				})
				visitorTeamId8 = foundVisitorTeam8.id
			}
			//
			//
			//
			//Now let's find the roster id's
			let homeTeamRosterId8
			let foundHomeTeamRosterId8
			if (homeTeam8 !== 'TBD') {
				try {
					foundHomeTeamRosterId8 = await Roster.findOne({
						leagueId: foundHomeTeam8.leagueId,
						teamId: homeTeamId8,
						divisionName: foundHomeTeam8.divisionName,
					}).orFail()
				} catch (err) {
					const error = new HttpError(
						'Could not find roster to obtain home rosterId. WITH divisions.  createGame 8',
						500
					)
					return next(error)
				}
				homeTeamRosterId8 = foundHomeTeamRosterId8.id
			}

			//// find visitor team roster id
			let visitorTeamRosterId8
			let foundVisitorTeamRosterId8
			if (visitorTeam8 !== 'TBD') {
				try {
					foundVisitorTeamRosterId8 = await Roster.findOne({
						leagueId: foundVisitorTeam8.leagueId,
						teamId: visitorTeamId8,
						divisionName: foundVisitorTeam8.divisionName,
					}).orFail()
				} catch (err) {
					const error = new HttpError(
						'Could not find roster to obtain home rosterId. WITH divisions.  createGame 8',
				...
				visitorTeamRosterId8 = foundVisitorTeamRosterId8.id
			}
			//
			//
			//
			...
			//Now we should have all the variables we need to create a new game
			if (homeTeam8 !== 'TBD' && visitorTeam8 !== 'TBD') {
				createdGame8 = new Game({
					...
					homeTeamName: homeTeam8,
					homeTeamId: homeTeamId8,
					homeRosterId: homeTeamRosterId8,
					homeTeamSeed: foundHomeTeam8.seed,
					visitorTeamName: visitorTeam8,
					visitorTeamId: visitorTeamId8,
					visitorRosterId: visitorTeamRosterId8,
					visitorTeamSeed: foundVisitorTeam8.seed,
					venueName: venue8,
					isCurrent: true,
				})
				try {
				...
				//If only the home team is TBD:
			} else if (homeTeam8 === 'TBD' && visitorTeam8 !== 'TBD') {
				console.log('28d')
				createdGame8 = new Game({
					leagueName: leagueName8,
					session: session8,
			...
			console.log('Game 8 has been added')
		} else if (!divisionName) {
			//
			//
			//
			...
			//
			//
			console.log('no divisions for this game 8')

			let homeTeamId8
			let foundHomeTeam8
			if (homeTeam8 !== 'TBD') {
				try {
					foundHomeTeam8 = await Team.findOne({
						leagueId: leagueId8,
						teamName: homeTeam8,
					}).orFail()
				} catch (err) {
					const error = new HttpError(
						'Could not find team to obtain home teamId. createGame 8',
						500
					)
					return next(error)
				}
			...
			if (visitorTeam8 !== 'TBD') {
				try {
					foundVisitorTeam8 = await Team.findOne({
						leagueId: leagueId8,
						teamName: visitorTeam8,
					}).orFail()
				} catch (err) {
					const error = new HttpError(
						'Could not find team to obtain visitor teamId. createGame 8',
						500
					)
					return next(error)
				}
			...
			if (homeTeam8 !== 'TBD') {
				try {
					...
					const error = new HttpError(
						'Could not find roster to obtain home rosterId. createGame 8',
						500
					)
					return next(error)
				}
			...
			if (visitorTeam8 !== 'TBD') {
				try {
					...
					const error = new HttpError(
						'Could not find roster to obtain visitor rosterId. createGame 8',
						500
					)
					return next(error)
				}
	...
	if (leagueName9) {
		...
		let leagueId9, divisionName
		let foundLeague9, foundLeague9WithDivisions
		try {
			foundLeague9 = await League.findOne({
				leagueName: leagueName9,
				session: session9,
				year: year9,
			}).orFail()
		} catch (err) {
		...
		leagueId9 = foundLeague9.id
		divisionName = foundLeague9.divisionName
		// We have our leagueId, so let's use that to find team names...
		//find home team Id
		if (divisionName) {
			console.log('we have divisions here!')
			try {
				foundLeague9WithDivisions = await League.find({
					leagueName: leagueName9,
					session: session9,
					year: year9,
				}).orFail()
				...
				const error = new HttpError(
					'Could not find league to obtain leagueId. WITH divisions. createGame 9',
					500
				)
				return next(error)
			}
			...
			//MATT START
			allTeamsInThisLeague = []
			for (let i = 0; i < foundLeague9WithDivisions.length; i++) {
				try {
					teams = await Team.find({
						leagueName: foundLeague9WithDivisions[i].leagueName,
						divisionName: foundLeague9WithDivisions[i].divisionName,
					}).orFail()
				} catch {}
				teams.forEach((team) => {
					allTeamsInThisLeague.push(team)
			...
			if (homeTeam9 !== 'TBD') {
				allTeamsInThisLeague.forEach(async (team) => {
					if (team.teamName === homeTeam9) {
						foundHomeTeam9 = team
					}
				})
				homeTeamId9 = foundHomeTeam9.id
			}
			//
			//
			//find visitor team id
			let visitorTeamId9
			let foundVisitorTeam9
			if (visitorTeam9 !== 'TBD') {
				allTeamsInThisLeague.forEach(async (team) => {
					//console.log(team.leagueName)
					console.log(team.teamName + '  ' + visitorTeam9)
					console.log(team.divisionName + ' ' + divisionName)

					if (team.teamName === visitorTeam9) {
						foundVisitorTeam9 = team
					}
				})
				visitorTeamId9 = foundVisitorTeam9.id
			}
			//
			//
			//
			//Now let's find the roster id's
			let homeTeamRosterId9
			let foundHomeTeamRosterId9
			if (homeTeam9 !== 'TBD') {
				try {
					foundHomeTeamRosterId9 = await Roster.findOne({
						leagueId: foundHomeTeam9.leagueId,
						teamId: homeTeamId9,
						divisionName: foundHomeTeam9.divisionName,
					}).orFail()
				} catch (err) {
					const error = new HttpError(
						'Could not find roster to obtain home rosterId. WITH divisions.  createGame 9',
						500
					)
					return next(error)
				}
				homeTeamRosterId9 = foundHomeTeamRosterId9.id
			}

			//// find visitor team roster id
			let visitorTeamRosterId9
			let foundVisitorTeamRosterId9
			if (visitorTeam9 !== 'TBD') {
				try {
					foundVisitorTeamRosterId9 = await Roster.findOne({
						leagueId: foundVisitorTeam9.leagueId,
						teamId: visitorTeamId9,
						divisionName: foundVisitorTeam9.divisionName,
					}).orFail()
				} catch (err) {
					const error = new HttpError(
						'Could not find roster to obtain home rosterId. WITH divisions.  createGame 9',
				...
				visitorTeamRosterId9 = foundVisitorTeamRosterId9.id
			}
			//
			//
			//
			...
			//Now we should have all the variables we need to create a new game
			if (homeTeam9 !== 'TBD' && visitorTeam9 !== 'TBD') {
				createdGame9 = new Game({
					...
					homeTeamName: homeTeam9,
					homeTeamId: homeTeamId9,
					homeRosterId: homeTeamRosterId9,
					homeTeamSeed: foundHomeTeam9.seed,
					visitorTeamName: visitorTeam9,
					visitorTeamId: visitorTeamId9,
					visitorRosterId: visitorTeamRosterId9,
					visitorTeamSeed: foundVisitorTeam9.seed,
					venueName: venue9,
					isCurrent: true,
				})
				try {
				...
				//If only the home team is TBD:
			} else if (homeTeam9 === 'TBD' && visitorTeam9 !== 'TBD') {
				console.log('29d')
				createdGame9 = new Game({
					leagueName: leagueName9,
					session: session9,
			...
			console.log('Game 9 has been added')
		} else if (!divisionName) {
			//
			//
			//
			...
			//
			//
			console.log('no divisions for this game 9')

			let homeTeamId9
			let foundHomeTeam9
			if (homeTeam9 !== 'TBD') {
				try {
					foundHomeTeam9 = await Team.findOne({
						leagueId: leagueId9,
						teamName: homeTeam9,
					}).orFail()
				} catch (err) {
					const error = new HttpError(
						'Could not find team to obtain home teamId. createGame 9',
						500
					)
					return next(error)
				}
			...
			if (visitorTeam9 !== 'TBD') {
				try {
					foundVisitorTeam9 = await Team.findOne({
						leagueId: leagueId9,
						teamName: visitorTeam9,
					}).orFail()
				} catch (err) {
					const error = new HttpError(
						'Could not find team to obtain visitor teamId. createGame 9',
						500
					)
					return next(error)
				}
			...
			if (homeTeam9 !== 'TBD') {
				try {
					...
					const error = new HttpError(
						'Could not find roster to obtain home rosterId. createGame 9',
						500
					)
					return next(error)
				}
			...
			if (visitorTeam9 !== 'TBD') {
				try {
					...
					const error = new HttpError(
						'Could not find roster to obtain visitor rosterId. createGame 9',
						500
					)
					return next(error)
				}
	...
	if (leagueName10) {
		...
		let leagueId10, divisionName
		let foundLeague10, foundLeague10WithDivisions
		try {
			foundLeague10 = await League.findOne({
				leagueName: leagueName10,
				session: session10,
				year: year10,
			}).orFail()
		} catch (err) {
		...
		leagueId10 = foundLeague10.id
		divisionName = foundLeague10.divisionName
		// We have our leagueId, so let's use that to find team names...
		//find home team Id
		if (divisionName) {
			console.log('we have divisions here!')
			try {
				foundLeague10WithDivisions = await League.find({
					leagueName: leagueName10,
					session: session10,
					year: year10,
				}).orFail()
				...
				const error = new HttpError(
					'Could not find league to obtain leagueId. WITH divisions. createGame 10',
					500
				)
				return next(error)
			}
			...
			//MATT START
			allTeamsInThisLeague = []
			for (let i = 0; i < foundLeague10WithDivisions.length; i++) {
				try {
					teams = await Team.find({
						leagueName: foundLeague10WithDivisions[i].leagueName,
						divisionName: foundLeague10WithDivisions[i].divisionName,
					}).orFail()
				} catch {}
				teams.forEach((team) => {
					allTeamsInThisLeague.push(team)
			...
			if (homeTeam10 !== 'TBD') {
				allTeamsInThisLeague.forEach(async (team) => {
					if (team.teamName === homeTeam10) {
						foundHomeTeam10 = team
					}
				})
				homeTeamId10 = foundHomeTeam10.id
			}
			//
			//
			//find visitor team id
			let visitorTeamId10
			let foundVisitorTeam10
			if (visitorTeam10 !== 'TBD') {
				allTeamsInThisLeague.forEach(async (team) => {
					//console.log(team.leagueName)
					console.log(team.teamName + '  ' + visitorTeam10)
					console.log(team.divisionName + ' ' + divisionName)

					if (team.teamName === visitorTeam10) {
						foundVisitorTeam10 = team
					}
				})
				visitorTeamId10 = foundVisitorTeam10.id
			}
			//
			//
			//
			//Now let's find the roster id's
			let homeTeamRosterId10
			let foundHomeTeamRosterId10
			if (homeTeam10 !== 'TBD') {
				try {
					foundHomeTeamRosterId10 = await Roster.findOne({
						leagueId: foundHomeTeam10.leagueId,
						teamId: homeTeamId10,
						divisionName: foundHomeTeam10.divisionName,
					}).orFail()
				} catch (err) {
					const error = new HttpError(
						'Could not find roster to obtain home rosterId. WITH divisions.  createGame 10',
						500
					)
					return next(error)
				}
				homeTeamRosterId10 = foundHomeTeamRosterId10.id
			}

			//// find visitor team roster id
			let visitorTeamRosterId10
			let foundVisitorTeamRosterId10
			if (visitorTeam10 !== 'TBD') {
				try {
					foundVisitorTeamRosterId10 = await Roster.findOne({
						leagueId: foundVisitorTeam10.leagueId,
						teamId: visitorTeamId10,
						divisionName: foundVisitorTeam10.divisionName,
					}).orFail()
				} catch (err) {
					const error = new HttpError(
						'Could not find roster to obtain home rosterId. WITH divisions.  createGame 10',
				...
				visitorTeamRosterId10 = foundVisitorTeamRosterId10.id
			}
			//
			//
			//
			...
			//Now we should have all the variables we need to create a new game
			if (homeTeam10 !== 'TBD' && visitorTeam10 !== 'TBD') {
				createdGame10 = new Game({
					...
					homeTeamName: homeTeam10,
					homeTeamId: homeTeamId10,
					homeRosterId: homeTeamRosterId10,
					homeTeamSeed: foundHomeTeam10.seed,
					visitorTeamName: visitorTeam10,
					visitorTeamId: visitorTeamId10,
					visitorRosterId: visitorTeamRosterId10,
					visitorTeamSeed: foundVisitorTeam10.seed,
					venueName: venue10,
					isCurrent: true,
				})
				try {
				...
				//If only the home team is TBD:
			} else if (homeTeam10 === 'TBD' && visitorTeam10 !== 'TBD') {
				console.log('210d')
				createdGame10 = new Game({
					leagueName: leagueName10,
					session: session10,
			...
			console.log('Game 10 has been added')
		} else if (!divisionName) {
			//
			//
			//
			...
			//
			//
			console.log('no divisions for this game 10')

			let homeTeamId10
			let foundHomeTeam10
			if (homeTeam10 !== 'TBD') {
				try {
					foundHomeTeam10 = await Team.findOne({
						leagueId: leagueId10,
						teamName: homeTeam10,
					}).orFail()
				} catch (err) {
					const error = new HttpError(
						'Could not find team to obtain home teamId. createGame 10',
						500
					)
					return next(error)
				}
			...
			if (visitorTeam10 !== 'TBD') {
				try {
					foundVisitorTeam10 = await Team.findOne({
						leagueId: leagueId10,
						teamName: visitorTeam10,
					}).orFail()
				} catch (err) {
					const error = new HttpError(
						'Could not find team to obtain visitor teamId. createGame 10',
						500
					)
					return next(error)
				}
			...
			if (homeTeam10 !== 'TBD') {
				try {
					...
					const error = new HttpError(
						'Could not find roster to obtain home rosterId. createGame 10',
						500
					)
					return next(error)
				}
			...
			if (visitorTeam10 !== 'TBD') {
				try {
					...
					const error = new HttpError(
						'Could not find roster to obtain visitor rosterId. createGame 10',
						500
					)
					return next(error)
				}
